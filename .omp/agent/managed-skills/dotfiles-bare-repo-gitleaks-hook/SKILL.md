...
### Commit sequencing: NEVER run `add`+`commit` concurrently on the dotfiles repo

The bare repo has a **single shared index**. Two concurrent `git add` + `git commit` invocations (e.g. two parallel tool calls, each staging one path and committing) race on that index: both `add`s land, then one commit grabs **both** files under whichever message won — producing a commit whose message doesn't match its contents ("Add X" containing X and Y), or a "nothing to commit" error in the other call. Keep the per-path commit hygiene by running `add` then `commit` **strictly sequentially, one path at a time** — never parallelize git operations against `.dotfiles`, even "one file per call". If a raced commit lands anyway: `git reset --soft HEAD~1` (safe: local repo, no push), unstage the stray path with `restore --staged`, then re-commit each path separately.
